...
colors=($blue $yellow $white $red $green $magenta $lgreen $cyan $purple $pink $orange_yellow $greenish_yellow)
...
msg $green "|| Cloning Toolchain ||" $white
	...
	cd toolchains
		...
		# Loop through each file (eva-gcc-arm, eva-gcc-arm64) to download.
		...
		echo "Directories '${dirgcc[0]}' and '${dirgcc[1]}' already exist. Skipping download."
	fi
	...
	if [[ ! -d "$CLANG_VERSION" ]]; then
		echo -e "$yellow Please choose a Clang version to download: $white"
		echo -e "  1) Weebx Clang"
		echo -e "  2) LLVM Clang (x86_64)"
		read -t 5 -p "Enter your choice (1 or 2, default: $CLANG_DEFAULT): " clang_choice

		case "$clang_choice" in
			1)
				echo -e "$blue << Downloading Weebx Clang... >> $white"
				wget "$(curl -s https://raw.githubusercontent.com/XSans0/WeebX-Clang/main/main/link.txt)" -O "weebx-clang.tar.gz"
				mkdir -p clang && tar -xf weebx-clang.tar.gz -C clang --strip-components=1 && rm -f weebx-clang.tar.gz
				echo -e "$green << Weebx Clang downloaded and extracted to 'clang' directory. >> $white"
				CLANG_VERSION="clang" # Define CLANG_VERSION to point to the directory
				;;
			2)
				echo -e "$blue << Downloading LLVM Clang (x86_64)... >> $white"
				wget "https://mirrors.edge.kernel.org/pub/tools/llvm/files/llvm-20.1.5-x86_64.tar.xz" -O "llvm-20.1.5-x86_64.tar.xz"
				mkdir -p clang && tar -xf "llvm-20.1.5-x86_64.tar.xz" -C clang --strip-components=1
				rm -f "llvm-20.1.5-x86_64.tar.xz"
				echo -e "$green << LLVM Clang downloaded and extracted to 'clang' directory. >> $white"
				CLANG_VERSION="clang" # Define CLANG_VERSION to point to the directory
				;;
			"") # If the user presses Enter (default)
				echo -e "$yellow Using default Clang: $CLANG_DEFAULT $white"
				if [[ "$CLANG_DEFAULT" == "weebx" ]]; then
					echo -e "$blue << Downloading Weebx Clang... >> $white"
					wget "$(curl -s https://raw.githubusercontent.com/XSans0/WeebX-Clang/main/main/link.txt)" -O "weebx-clang.tar.gz"
					mkdir -p clang && tar -xf weebx-clang.tar.gz -C clang --strip-components=1 && rm -f weebx-clang.tar.gz
					echo -e "$green << Weebx Clang downloaded and extracted to 'clang' directory. >> $white"
					CLANG_VERSION="clang"
				elif [[ "$CLANG_DEFAULT" == "llvm" ]]; then
					echo -e "$blue << Downloading LLVM Clang (llvm-20.1.5-x86_64)... >> $white"
					wget "https://mirrors.edge.kernel.org/pub/tools/llvm/files/llvm-20.1.5-x86_64.tar.xz" -O "llvm-20.1.5-x86_64.tar.xz"
					mkdir -p clang && tar -xf "llvm-20.1.5-x86_64.tar.xz" -C clang --strip-components=1
					rm -f "llvm-20.1.5-x86_64.tar.xz"
					echo -e "$green << LLVM Clang downloaded and extracted to 'clang' directory. >> $white"
					...
					echo -e "$red Error: Invalid default Clang specified. Please check the script. $white"
					exit 1
				fi
				;;
			*)
				echo -e "$red Invalid choice. Using default Clang: $CLANG_DEFAULT $white"
				if [[ "$CLANG_DEFAULT" == "weebx" ]]; then
					echo -e "$blue << Downloading Weebx Clang... >> $white"
					wget "$(curl -s https://raw.githubusercontent.com/XSans0/WeebX-Clang/main/main/link.txt)" -O "weebx-clang.tar.gz"
					mkdir -p clang && tar -xf weebx-clang.tar.gz -C clang --strip-components=1 && rm -f weebx-clang.tar.gz
		...
		echo -e "$green Clang directory '$CLANG_VERSION' already exists. Skipping download. $white"
	fi
...
echo "2. McQ-KSU"
echo "3. McQ-NetHunter"
echo "4. McQ-Clang"
echo "5. McQ-Legacy"
echo "6. McQ-Nethunter-Legacy"
echo "7. McQ-KSU-Legacy"
echo "8. McQ-veux"
echo "9. McQ-KSU-veux"
echo "10. Nebula"
...
read -t 5 -p "Enter your choice [1-7]: " choice || {
    ...
    echo -e "$lgreen Ak3 choice to $AK3_DEFAULT. $white"
    choice=$AK3_DEFAULT
    }
#fi
    ...
    case $choice in
        ...
        3)
            ...
            ;;
        4)
            git clone --depth=1 https://github.com/dev-sm8350/AnyKernel3 -b McQ-Clang "${ANYKERNEL3_VERSION}"            
            ;;
        5)
        ...
        7)
            ...
            ;;
        8)
            git clone --depth=1 https://github.com/dev-sm8350/AnyKernel3 -b McQ-veux "${ANYKERNEL3_VERSION}"
            ;;
        9)
            git clone --depth=1 https://github.com/dev-sm8350/AnyKernel3 -b McQ-KSU-veux "${ANYKERNEL3_VERSION}"
            ;;
        10)
            git clone --depth=1 https://github.com/dev-sm8350/AnyKernel3 -b Nebula "${ANYKERNEL3_VERSION}"
            ;;
        11)
...
config_generator()
{
	...
	if [[ ! -f arch/arm64/configs/$DFCF ]]; then
		...
		bash scripts/gki/generate_defconfig.sh lahaina-qgki_defconfig vendor/lahaina_QGKI.config vendor/debugfs.config vendor/oplus_QGKI.config veux-qgki_defconfig
...
kernel_builder()
{
	...
	*************Build Triggered*************
	...
	CI: $KBUILD_HOST
	Distro: $(lsb_release -d | sed 's/^Description:\s*//')
	Core count: $(nproc)
	Device: $DEVICENAME
	Codename: $CODENAME
	Compiler: $C_NAME
...
zipper()
{
	...
	find "$DTB_PATH"/vendor/*/* -name '*.dtb' -exec cat {} + > "$AK3_DIR"/dtb # Combine all vendor DTBs into one file
	...
	if [ "$DRM_AS_MODULE" = "1" ]; then
		LOCAL_VERSION_CUT=$(echo "$CONFIG_LOCALVERSION" | cut -c 8-)
		zip -r9 "${ANYK_VERSION}-r${RELEASE}-${MOBILEDEVICE}-${BUILD_TIME}-${LOCAL_VERSION_CUT}-${LAST_HASH}.zip" ./*
	...
	file_path="$(pwd)/${ANYK_VERSION}-r${RELEASE}-${MOBILEDEVICE}-${BUILD_TIME}-signed.zip"
	...
	upload_mega() {
		...
		# Uploads the file
		megaput "$file_path" -u "$email" -p "$password"
		countdown 3
		echo -e "$reset 👍 Done $white"
	}
	# MEGAUP END\*
	...
	upload_pixeldrain() {
		...
		if [ "$status_code" -eq 201 ]; then
			...
			echo -e "$cyan Download link: https://pixeldrain.com/u/$file_id $white"
			countdown 3
			echo -e "$reset 👍 $white"
		else
	...
	# */GOFILE
	# Function to upload to GoFile.io
	upload_gofile() {
		# Find the best server using GoFile API
		SERVER=$(curl -s https://api.gofile.io/servers | jq -r '.data.servers[0].name')

		# Print error message if server information cannot be retrieved
		if [ -z "$SERVER" ]; then
			echo "Error: Server information could not be retrieved"
			return 1
		fi
		...
		LINK=$(curl -# -F "file=@$file_path" "https://${SERVER}.gofile.io/uploadFile" | jq -r '.data.downloadPage')

		# Show download link if upload is successful, print error message if upload fails
		if [ -n "$LINK" ]; then
			echo "✅ File uploaded successfully!"
			echo -e "$cyan Download Link: $LINK $white"
			countdown 3
			echo -e "$reset 👍 $white"
			return 0
		else
	...
	# Ask the user which upload method they want to use
	echo -e "$yellow \n👉 Choose upload method: (1) Mega, (2) Pixeldrain, (3) GoFile.io, (q) Quit $white"
	read -t 5 -p "$(tput setaf 171) Enter your choice: " choice || {
		echo -e "$cyan Timeout of 5 seconds reached. No input received. $white"
	...
	case "$choice" in
		1)
			...
			read -t 3 -p "$(tput setaf 171) Enter your answer: " answer || {
				echo -e "$cyan Timeout of 3 seconds reached. No input received. $white"
				echo -e "$lgreen Setting choice to y. $white"
				answer="y"
			}
			if [[ "$answer" == "y" ]]; then
				upload_mega
			else
				echo -e "$reset ❌ Upload cancelled. $white"
			fi
			;;
		2)
			# Upload to Pixeldrain
			echo -e "$yellow \n👉 Do you want to upload the file $file_path to Pixeldrain? (y/n) $white"
			read -t 3 -p "$(tput setaf 171) Enter your answer: " answer || {
				echo -e "$cyan Timeout of 3 seconds reached. No input received. $white"
				echo -e "$lgreen Setting choice to y. $white"
				answer="y"
		...
		3)
			# Upload to GoFile.io
			echo -e "$yellow \n👉 Do you want to upload the file $file_path to GoFile.io? (y/n) $white"
			read -t 3 -p "$(tput setaf 171) Enter your answer: " answer || {
				echo -e "$cyan Timeout of 3 seconds reached. No input received. $white"
				echo -e "$lgreen Setting choice to y. $white"
				answer="y"
			...
			if [[ "$answer" == "y" ]]; then
				if upload_gofile; then
					: # Success, nothing more to do
				else
					echo -e "$reset ❌ Upload failed. $white"
				fi
			else
				echo -e "$reset ❌ Upload cancelled. $white"
			fi
			;;
		q)
			echo -e "$reset Upload process terminated. $white"
			;;
		*)
			echo -e "$reset Invalid choice. $white"
			;;
	esac
		...
		"--device="*)
			...
			case $CODE_NAME in
				lahaina)
					...
					;;
				veux)
					DEVICENAME='veux peux holi common qgki kernel'
					CODENAME='veux'
					SUFFIX='qgki'
					;;
				*)
