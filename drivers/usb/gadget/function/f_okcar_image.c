...
static void acc_complete_intr(struct usb_ep *ep, struct usb_request *req)
{
	struct acc_dev *dev = ep->driver_data;
				...
				struct usb_endpoint_descriptor *intr_desc)
{
	...
	DBG(cdev, "usb_ep_autoconfig for ep_intr got %s\n", ep->name);
	ep->driver_data = dev;		/* claim the endpoint */
	dev->ep_intr = ep;
	...
	for (i = 0; i < INTR_REQ_MAX; i++) {
		req = acc_request_new(dev->ep_intr, INTR_BUFFER_SIZE);
		if (!req)
			goto fail;
		req->complete = acc_complete_intr;
		req_put(dev, &dev->intr_idle, req);
	}
...
fail:
	...
	while ((req = req_get(dev, &dev->intr_idle)))
		acc_request_free(req, dev->ep_intr);
	return -1;
}
			...
			struct usb_function *f, bool configfs)
{
	...
	if (configfs) {
		...
		dev->acc_interface_desc.iInterface = 0;
		dev->cdev = c->cdev;
	}
	...
	ret = create_bulk_endpoints(dev, &(dev->acc_fullspeed_in_desc),
			&(dev->acc_fullspeed_out_desc), &(dev->acc_intr_desc));
	if (ret)
		return ret;
...
static struct usb_function_instance *acc_alloc_inst(void)
{
	...
	dev->acc_interface_desc.bNumEndpoints			= 3;
	dev->acc_interface_desc.bInterfaceClass			= USB_CLASS_STILL_IMAGE;
	...
	dev->acc_intr_desc.bLength						= USB_DT_ENDPOINT_SIZE;
	dev->acc_intr_desc.bDescriptorType				= USB_DT_ENDPOINT;
	dev->acc_intr_desc.bEndpointAddress				= USB_DIR_IN;
	dev->acc_intr_desc.bmAttributes					= USB_ENDPOINT_XFER_INT;
	dev->acc_intr_desc.wMaxPacketSize				= __constant_cpu_to_le16(INTR_BUFFER_SIZE);
	dev->acc_intr_desc.bInterval					= 10;
	...
	dev->fs_acc_descs[3] = (struct usb_descriptor_header *) &(dev->acc_intr_desc);
	dev->fs_acc_descs[4] = NULL;
...
DECLARE_USB_FUNCTION_INIT(okcar_image, acc_alloc_inst, acc_alloc);
MODULE_LICENSE("GPL");
